...
export class AuthService extends GenericService {
  ...
  login(payload): Observable<ApiResult<AuthResponse>> {
    ...
    return this.get(`${this.controller}login?`+queryParams, null, false);
  }
  register(payload): Observable<ApiResult<AuthResponse>> {
    return this.post(`${this.controller}register?`, payload);
  }
  ...
  refreshToken(): Observable<ApiResult<AuthResponse>> {
    ...
    return this.post<ApiResult<AuthResponse>>(`${this.controller}refresh`, { refreshToken }).pipe(
      tap(result => {
        if (result.isSuccess && result.data) {
          this.storeTokens(result.data.token, result.data.refreshToken);
